...
```{r}
list.files(path = "All_inputs/Microarray_signatures")
names_sig <- c("qNSC", "aNSC", "TAP", "iNB", "NB")


Population_signatures <- lapply(X = paste0("All_inputs/Microarray_signatures/", #Use list_files again
                                 names_sig, 
                                 "_signature.csv"),
                      FUN = read_csv)
...
if(length(vec_max) > 1){
  my_max <- vec_max[1] # comment gere which.max dans cette situation
  warnings("Attention égalité dans les maximums")
  
  #alternative
  stop("Egalite dans les max, gerer autrement l'attribution")
  
} else {
  my_max <- vec_max
}
# test <- apply(FBL_NPC@meta.data[, paste0(names_sig2, method_UC)],
#               1, #1 for rows ; 2 for columns
#               function(line) {
#                 colnames(FBL_NPC@meta.data[, paste0(names_sig2,
#                                           method_UC)])[which.max(line)]})
# test <- gsub(pattern = method_UC, replacement = "", test)
# df <- cbind.data.frame(test, score = UCell_scores$Identity)
# identical(df$test, df$score)

# --> all clear donc on peut s'affranchir du passage avec UCell dataframe

# No two max values were identical when "which.max" was replaced with "function(vec){which(vec == max(vec))} and the lign was stored in a vector instead of a data.frame

#Réfléchir à un scatterplot de la variance et de
